...
로컬 실행 기록에 남은 태스크 파일은 76개다. 그중 답 형식을 한 번이라도 언급한 것은
**13개**이고, 그 13개는 전부 플래너가 실패해 런타임이 요청 전문을 그대로 태스크로
만들어버린 경우다(`squad/ikkim/tasks/`). **플래너가 실제로 쓴 나머지 63개는 0개**다
...
| systemPrompt 길이 | Router 4,452 / Coder 7,698 / Solver 6,312자 | Router는 v1의 3.5분의 1, Coder·Solver는 약 절반. 공유 Layer 1 없음 |
...
| Router의 복사 규칙 | `--- ITEM ---` 줄 아래 전부 verbatim, coding만 약 32,000자 | 계약이 통째로 워커에게 도착해야 한다 |
| coding이 예산을 넘길 때 복사 순서 | ① REQUIRED OUTPUT 블록 ② 이슈 본문 ③ 발췌 | 예산이 모자라면 발췌가 잘려야지 계약이 잘리면 안 된다. 계약을 잃으면 맞는 패치도 0점이다 |
| one-shot 프롬프트 | `PLANNER:` 문단 + `--- ITEM ---` + `{{TASK}}` | 라우팅을 요청 본문에도 박아 두 번 말한다. 풀이 지침은 전부 systemPrompt로 옮겨 복사 비용을 없앴다 |
| 에이전트 배열 순서 | `[Router, Coder, Solver]` | 플래너가 플랜을 못 만들면 런타임이 non-planner 자리마다 태스크를 하나씩 만들고, `extract.py`는 배열 뒤쪽부터 읽는다. 그 경우 **Solver 출력이 채점된다** — 147문항 중 109개가 Solver 트랙이라 맞는 선택이지만, 순서가 의미를 갖는다는 것 자체를 기록해 둔다 |

### 예상 비용 (연습 세트 실측 크기 × hidden 문항 수)

연습 세트 121문항을 새 프롬프트로 실제 합성해서 잰 값이다.

| | v1 실측 | v2 추정 | 1등 실측 |
|---|---|---|---|
| requests | 1,192 | **294** | 327 |
| input | 5,368,135 | **약 1.50M** | 2,033,190 |
| output | 775,612 | **약 0.62M** | 496,359 |
| 합계 | 6.14M | **2.12M** | 2.53M |

합성 실측(중앙값): coding 요청 65,060B / 복사분 32,000B(상한에 걸림),
math 요청 1,005B / 복사분 493B, generic 요청 1,484B / 복사분 919B.
one-shot 본문이 `--- ITEM ---` 위로 올라가 복사에서 빠진 덕에 math·generic 요청이
v1의 3,788·3,958B에서 1,005·1,484B로 줄었다.

**단서 하나.** 이 계산은 문항당 모델 호출을 **2회**(Router 1 + 워커 1)로 본다.
`squad/test_2/logs/events.jsonl`에서 실행당 `squad:planning-started`가 1회인 것은 확인했지만,
플래너가 `create_task` 결과를 받고 한 턴을 더 도는지는 확인하지 못했다. 돈다면
requests 441, input 약 2.2M이 된다 — coding 요청을 플래너가 두 번 읽기 때문이다.
